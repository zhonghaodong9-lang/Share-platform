...
def format_daily_report(market_data, money_flow_data, overseas_data, reports_data, sentiment, mapping, ai_summary):
    """
    100% 兼容微信暗黑模式 (Dark Mode) 的移动端 4 大模块超清晰智投排版
    对所有文本标签强制指定 color:#0f172a / color:#1e293b，彻底解决白底白字隐形 Bug。
    """
...
<td style="padding:6px 4px;text-align:right;color:#64748b;">{item['volume_amount']:.0f}亿</td>
</tr>"""

    # 2. 模块二：指定 6 大 ETF (暗黑模式兼容)
    etf_html = ""
    for item in target_etfs:
        chg = item.get("change_rate", 0.0)
        c = "#dc2626" if chg >= 0 else "#16a34a"
        bg = "#fee2e2" if chg >= 0 else "#dcfce7"
        etf_html += f"""<div style="background:#ffffff;padding:8px 10px;margin-bottom:6px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px;display:flex;justify-content:space-between;align-items:center;">
<div style="color:#0f172a;"><b style="color:#0f172a;">{item['name']}</b> <span style="color:#64748b;font-size:10px;">({item['code']})</span></div>
<div><b style="color:#0f172a;margin-right:6px;">{item['volume_amount']:.2f}亿</b><span style="background:{bg};color:{c};padding:2px 6px;border-radius:4px;font-size:11px;"><b style="color:{c};">{chg:+.2f}%</b></span></div>
</div>"""
...
<div style="color:#475569;">领涨: <b style="color:#0284c7;">{item['leader_code']}</b></div>
    ...
    # 4. 模块四：成交额 Top 10 个股 (暗黑模式兼容，10 只个股 100% 完整输出)
    top10_html = ""
...
<div style="display:flex;justify-content:space-between;margin-top:4px;font-size:11px;color:#475569;">
<div style="color:#475569;">成交额: <b style="color:#0f172a;font-size:12px;">{item['volume_amount']:.2f} 亿</b></div>
...
<div style="font-size:15px;font-weight:900;color:#dc2626;margin-top:2px;">{tot_v:.2f} 亿元 ({tot_v/10000:.4f} 万亿)</div>
</div>
...
<thead><tr style="background:#f1f5f9;font-size:10px;color:#475569;text-align:left;"><th style="padding:4px;color:#475569;">指数</th><th style="padding:4px;text-align:right;color:#475569;">点位</th><th style="padding:4px;text-align:right;color:#475569;">涨跌</th><th style="padding:4px;text-align:right;color:#475569;">成交</th></tr></thead>
<tbody>{idx_html}</tbody>
</table>
</div>

<div style="background:#ffffff;padding:10px;border-radius:8px;margin-top:8px;">
<div style="font-size:13px;font-weight:900;color:#0f172a;border-left:4px solid #0284c7;padding-left:6px;margin-bottom:6px;">📊 模块二：指定 6 大核心 ETF 成交量监控</div>
{etf_html}
</div>
...
<div style="background:#ffffff;padding:10px;border-radius:8px;margin-top:8px;">
<div style="font-size:13px;font-weight:900;color:#0f172a;border-left:4px solid #8b5cf6;padding-left:6px;margin-bottom:6px;">💥 模块四：全市场成交额 Top 10 个股及行业</div>
{top10_html}
</div>
